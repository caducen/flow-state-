          ...
          activeTasks.map(task => (
            ...
            >
              ...
              <div className="flex items-start gap-3">
                {/* Today's Plan star - made larger and more visible */}
                <button
                  onClick={(e) => {
                    e.stopPropagation()
                    handleToggleTodayTask(task.id)
                  }}
                  title={task.isTodayTask ? "Remove from today's plan" : "Add to today's plan"}
                  className={`mt-0.5 transition-all duration-200 ${
                    task.isTodayTask
                      ? 'text-amber-glow scale-110 drop-shadow-[0_0_4px_rgba(245,158,11,0.5)]'
                      : 'text-ink-faint/60 hover:text-amber-glow hover:scale-110'
                  }`}
                >
                  <svg className="w-5 h-5" fill={task.isTodayTask ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
